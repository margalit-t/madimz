...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.querySelectorAll('.product-gallery').forEach(wrapper => {
    ...
    const main   = wrapper.querySelector('.swiper-pdt-container');
    const thumbs = wrapper.querySelector('.swiper-thumbs');

    if (!main || !thumbs) return;

    const thumbsGallery = new Swiper(thumbs, {
      slidesPerView: 5,
      spaceBetween: 5,
      watchSlidesProgress: true,
    });

    const mainGallery = new Swiper(main, {
      slidesPerView: 1,
      spaceBetween: 0,
      rtl: true, // VERY IMPORTANT on RTL sites
      navigation: {
...
function setupLightbox(images) {
    ...
    document
      ...
      .forEach(link => {
          link.onclick = null; // מנקה מאזין קודם
      });

    const pswpElement = document.querySelector('.pswp');
    if (!pswpElement || typeof PhotoSwipe === 'undefined') return;

    document.querySelectorAll('.swiper-pdt-container .swiper-slide a')
        .forEach((link, index) => {
